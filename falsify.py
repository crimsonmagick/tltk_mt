...
def sim_and_return_rob(z, *params):
    ...
    model, step, inp_range, simulation_time, rt, eng = params

    for i in range(0, len(cur_sample)):
        cur_sample[i] = min(max(inp_range[0], cur_sample[i]), inp_range[1])

    signal = computeInputSignal.compute_input_signal("pchip", cur_sample,
                                                     inp_range, len(cur_sample), simulation_time, step)

    # Simulate the model
    time_stamps, internal_states, output = systemSimulator.simulate_system(eng, model, simulation_time, signal)
    ...
    traces = {'speed': speed_data, 'rpm': rpm_data}
...
def falsify(model, step, inp_range, simulation_time, root):
    # Initialize matlab engine
    engine = systemSimulator.init_engine()
    params = (model, step, inp_range, simulation_time, root, engine)

...
cdef extern from "backend.h":
    float* higher_dim_pred_threaded(int n, int m, double* q,double* l, double* u, double **init_A, double **init_P, float** traces, long length);
...
def py_higher_dim_threaded(int n, int m, list q, list l, list u, list init_A, list init_P, list traces, long length, list results) -> float[::1]:
    cdef double* c_q
    cdef double* c_l
    cdef double* c_u
    cdef double** A
    ...
    for i in xrange(len(u)):
        c_u[i] = u[i]
    result = higher_dim_pred_threaded(n, m, c_q, c_l, c_u, A, P, c_traces, length)
    for i in xrange(length):
        results[i] = result[i]
    with nogil:
        free(c_q)

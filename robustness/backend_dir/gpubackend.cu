...
__global__  void gpu_finally(long length,long sub_length,float lower_time_bound,float upper_time_bound ,float* robustness, float* time_stamps, float* finally_robustness){
    long current_time_step;
    long start_time_step = (blockIdx.x*blockDim.x + threadIdx.x) * sub_length;
    if (start_time_step < length){
        for(current_time_step = 0; current_time_step < sub_length && (current_time_step + start_time_step) < length; current_time_step++){
...
void predicate_setup(float* cpu_traces, float A, float bound,long length){
    ...
    long blocks = (length / 1024) + 1; //divide by 1024 to the 5.2 compute standard of my 980ti and add 1 encase the division rounded down
    long sub_length = (blocks / 1024) + 1;
    if(blocks > 1024){
        blocks = 1024;
    }
    ...
    predicate<<<blocks, 1024>>>(length, sub_length,A,bound,gpu_traces);
    cudaMemcpy(cpu_traces, gpu_traces, length*sizeof(float), cudaMemcpyDeviceToHost);
    cudaFree(gpu_traces);
}

void finally_setup(float* cpu_traces, float* cpu_time_stamps, float* results,float lower_time_bound, float upper_time_bound,long length){
    float *gpu_traces;
    ...
    gpu_finally<<<blocks,1024>>>(length,sub_length,lower_time_bound, upper_time_bound,gpu_traces,gpu_time_stamps,gpu_results);
...
int main(){
    ...
    finally_setup(traces,time_stamps,results,0,100,length);
    clock_t end = clock();
